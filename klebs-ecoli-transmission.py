...
#Argparse
parser = argparse.ArgumentParser(description="Agent Based Models of AMR introduction and spread in a hospital ward. \n\nAuthor Tom Crellen (tomcrellen@gmail.com) MORU Postdoc. \n \n Models can 1) simulate cohort study or 2) calculate R0")
#determine which model to run
parser.add_argument('-m', '--mode', default=None, required=False, dest="mode", metavar="", help="[cohort / r0] Model section, cohort study or R0 simulator")
...
parser.add_argument('-r', '--entryrate', default =3, required=False, dest="entry", metavar="", help="Entry rate of patients per day, Poisson rate parameter (3)")
parser.add_argument('-k', '--hgtkleb', default=0.05, required=False, dest="HGT_kleb", metavar="", help="Probability of horizontal gene tranfer from K. pneumoniae to E. coli (0.05)")
parser.add_argument('-p','--masskleb', default=0.05, required=False, dest="PMA_kleb", metavar="", help="Probability of person-to-person transmission of K. pneumoniae (0.05)")
parser.add_argument('-e', '--hgtecoli', default= 0.0005, required=False, dest="HGT_ecoli", metavar="", help="Probability of horizontal gene transfer from E. coli to K. pneumoniae (0.0005)")
parser.add_argument('-c', '--massecoli', default=0.01, required=False, dest="PMA_ecoli", metavar="", help="Probability of person-to-person transmission of E. coli (0.01)")
...
parser.add_argument('-y', '--importecoli', default=0.3, required=False, dest="import_ecoli", metavar="", help="Probability that patient is colonized with E. coli on admission (imported case) (0.3)")
...
import_ecoli = float(args.import_ecoli)
...
class ward:
        def __init__(self, n_iterations=n_iterations, entry_rate=entry_rate, beds=beds, los_dist=los_dist, HGT_klebs=HGT_klebs, PMA_klebs=PMA_klebs, HGT_ecoli=HGT_ecoli, PMA_ecoli=PMA_ecoli, import_klebs=import_klebs, import_ecoli=import_ecoli):
                self.n_iterations = n_iterations
                self.los = numpy.array(los_dist)
                self.patients = {}
                self.entry_rate = entry_rate
                self.entry_risk_klebs = import_klebs
                self.entry_risk_ecoli = import_ecoli
...
class R0:
        def __init__(self, beds=beds, los_dist=los_dist, p_acquire=PMA_klebs):
                self.los = numpy.array(los_dist)
                self.patients = {}
                self.beds = beds
                self.p_acquire = p_acquire
                self.R = 0
                self.patient_zero = numpy.random.choice(self.los)
                self.occupied_beds = []

        def admit(self):
                for n in range(1, self.beds):
                        #give characteristics to new patients
                        name = "1"+"."+str(n)
                        los = numpy.random.choice(self.los)
                        discharge_day = 1+los
                        #patient dict. ID is key, values are day of entry, discharge, klebs dict, ecoli dict
                        self.patients[name] = [0, discharge_day, 0]
                
                #give characterics to colonised patient
                self.patients["zero"] = [0, self.patient_zero+1, 1]

                #list of occupied beds
                self.occupied_beds = self.patients.keys()

        def spread(self):
                for day in range(1, self.patient_zero+1):
                        
                        #remove patients where discharge day == current day
                        remove = [bed for bed, date in self.patients.items() if date[1]==day]
                        self.occupied_beds = [bed for bed in self.occupied_beds if bed not in remove]
                        
                        #admit new patients when old ones have been discharged
                        ...
                        #check which patients are susceptible and infected
                        for agent in self.occupied_beds:

                                if self.patients[agent][2] == 0:   
                                        susceptible.append(agent)

                        #infection is a Bernoulli trial
                        outcome = numpy.random.binomial(n=1, p=self.p_acquire, size=len(susceptible))
                        #indexes of successful transmission events (1)                     
                        outcome_index = [o for o,x in enumerate(outcome) if x==1]
                        #update patient dictionary
                        if outcome_index:
                                for j in outcome_index:
                                        self.patients[susceptible[j]][2] = 1
                        
                        #add to R the number of new colonizations
                        self.R += sum(outcome)

                print str(self.patient_zero) + "\t" +  str(self.R)
#Run model based on mode argument
#Cohort simulation
if str.lower(args.mode)=="cohort":
        run=ward()
...
#R0 simulation
elif str.lower(args.mode)=="r0":
        run=R0()
        run.admit()
        run.spread()

else:
        print "not a valid model selection. Use -m [cohort / R0]"
